...
class DataProxy(Debugged, Logged, PyThread):
    ...
    def __init__(self, config_file, data_logger, logfile, logdir):
        ...
        for srv in self.Config["Servers"]:
            ...
            self.Servers[port] = server = VirtualServer(self, self.DataLogger, srv, logdir)
...
if __name__ == "__main__":
    ...
    try:
        ...
        tm = DataProxy(config_file, data_logger, logs.logfile, logdir)
        data_logger.start()
        ...
        if "WebGUI" in config:
            cfg = config["WebGUI"]
            static_location = cfg.get("StaticLocation", "./product/static")
            ...
            gui = DataProxyWebInterface(tm, title, data_logger, static_location, correlations_file)
            port = cfg["port"]
            tls = "tls" in cfg
